...
public class MToast {
    ...
    private MToast(Context context, CharSequence text, int HIDE_DELAY){
        manger = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);

        this.text = text;

        if(HIDE_DELAY == MToast.LENGTH_SHORT)
            this.time = 2000L;
        else if(HIDE_DELAY == MToast.LENGTH_LONG)
            this.time = 3500L;
    ...
    public static MToast makeText(Context context, String text, int HIDE_DELAY){
        MToast toast = new MToast(context, text, HIDE_DELAY);
        return toast;
    }
