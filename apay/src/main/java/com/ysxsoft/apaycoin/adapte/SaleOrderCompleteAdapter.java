 ...
 * 公司：郑州亿生信科技有限公司
 */
public class SaleOrderCompleteAdapter extends ListBaseAdapter<SaleBuyOrderBean.DataBean> {
    private OnDeleteListener onDeleteListener;

    public SaleOrderCompleteAdapter(Context context) {
        super(context);
    }
...
//        tv_trade_type.setText(dataBean.);  //交易类型
        ...
        tv_sale_time.setText(dataBean.getAdd_time());//发起时间
...
//                submitData(oid);
                if (onDeleteListener!=null){
                    onDeleteListener.deleteData(position);
                }
            }
        });
