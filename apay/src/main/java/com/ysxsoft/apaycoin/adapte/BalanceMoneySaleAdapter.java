 ...
 * 公司：郑州亿生信科技有限公司
 */
public class BalanceMoneySaleAdapter extends ListBaseAdapter<BalanceMoneyBuyOrSaleBean.DataBean> {
    private int type = -1;//支付方式  1余额 2现金
    private BalanceMoneyBuyOrSaleAdapter.OnBalanceMoneyBuyAndSale onBalanceMoneyBuyAndSale;

    public BalanceMoneySaleAdapter(Context context) {
        super(context);
    }
    ...
    public void onBindItemHolder(final SuperViewHolder holder, final int position) {
        ...
        switch (dataBean.getZf_type()) {
            case 0 + "":
                ...
                tv_balance_money_buy.setText("余额出售");
                tv_buy_sale.setText("现金出售");
                break;
            case 1 + "":
                tv_balance_money_buy.setVisibility(View.VISIBLE);
...
//        tv_current_price.setText(dataBean.getMoney());
    ...
    public void setOnBalanceMoneyBuyAndSale(BalanceMoneyBuyOrSaleAdapter.OnBalanceMoneyBuyAndSale onBalanceMoneyBuyAndSale){
